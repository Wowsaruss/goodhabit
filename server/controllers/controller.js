module.exports = {
       ...
       submitGoalData: (req,res) => {
            const db = req.app.get('db')
            let {oneday, threeday, oneweek, twoweek, threeweek, onemonth, twomonth, threemonth,
                fourmonth, fivemonth, sixmonth, oneyear} = req.body
       ...
       getJournalData: (req, res) => {
            req.app.get('db').get_journal([req.userid]).then(journal => {
                res.status(200).send(journal);
            }).catch((err) => {console.log(err)})
        },
    
       getGoalsData: (req, res) => {
            req.app.get('db').get_goals([req.userid]).then(goals => {
